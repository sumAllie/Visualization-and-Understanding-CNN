...
from models import AlexConv, AlexDeconv
import utils
...
def vis_layer(layer, conv, deconv):
    """
    ...
    deconv_output = deconv(new_feat_map, layer, mark, conv.pool_locs)

    m = deconv_output.data.numpy()[0]
...
if __name__ == '__main__':
    ...
    alex_conv.eval()
    store(alex_conv)
    conv_output = alex_conv(img)
    pool_locs = alex_conv.pool_locs
    print('Predicted:', utils.decode_predictions(conv_output, top=3)[0])
    ...
    img = cv2.resize(img, (224, 224))
    plt.imshow(img)
    for idx, layer in enumerate([0, 3, 6, 8, 10]):
        # for idx, layer in enumerate(vgg16_conv.conv_layer_indices):
        plt.subplot(2, 4, idx + 2)
        img, activation = vis_layer(layer, alex_conv, alex_deconv)
